    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ background: "#fff", flexShrink: 0 }}>
        ...
        <div style={{ padding: "4px 16px 16px", display: "flex", gap: 14, alignItems: "center" }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: `linear-gradient(135deg,${G},${GD})`, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 26, fontWeight: 700, color: "#fff" }}>{displayInitials}</span>
          </div>
          <div>
            <p style={{ fontSize: 18, fontWeight: 700, color: T1, margin: "0 0 4px" }}>{displayName}</p>
            <p style={{ fontSize: 13, color: T2, margin: 0 }}>{displayEmail}</p>
          </div>
        </div>
      </div>
      <div style={{ flex: 1, overflow: "auto", padding: "12px 16px", background: BG }}>
        ...
        <button
          ...
          style={{ width: "100%", padding: "14px 16px", display: "flex", alignItems: "center", gap: 12, background: "#FEE2E2", border: "none", borderRadius: 14, cursor: "pointer" }}
        >
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: ERR }}>Sair da conta</span>
        </button>
      </div>
    </div>
  );
